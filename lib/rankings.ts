...
export async function fetchRankings(limitParam?: string | null): Promise<RankingsResponse> {
  ...
  const rankings: RankingEntry[] = members.map((m, idx) => {
    ...
    const source   = colonIdx === -1 ? m   : m.slice(0, colonIdx);
    const id       = colonIdx === -1 ? ''  : m.slice(colonIdx + 1);
